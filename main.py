...
import random
import hashlib
from DatabaseManager import localizar_cliente_db, localizar_agencia_conta_db, atualizar_transferencia, atualizar_deposito_bancario, atualizar_saque, armanezar_transacao, consultar_saldo_bancario, cadastrar_cliente
...
class ContaCorrente:
    ...
    def _validar_cpf(self) -> str:
        ...
        while True:
            ...
            cpf_formatado = '{}.{}.{}-{}'.format(self._cpf[:3], self._cpf[3:6], self._cpf[6:9], self._cpf[9:])
            localizar_cliente = self._localizar_cliente(cpf_formatado)
            if localizar_cliente != 'Cliente não encontrado':
                ...
                if len(str(self._cpf)) == 11 and self._cpf.isnumeric():
                    self._cpf = cpf_formatado
                    os.system('cls')
    ...
    def _validar_profissao(self) -> str:
        while True:
            if not (profissao := input('Digite a sua profissão: ').strip()):
                return None
            else:
                if len(profissao) < 4 or not profissao.isalpha():
    ...
    def _validar_renda(self) -> float | None:
        while True:
            self._renda = input('Digite a sua renda atual: ').strip()
            if not self._renda.strip():
                return None
            else:
                verificar_renda = self._renda.replace(',', '_').replace('.', '').replace('_', '.')
                if float(verificar_renda) == 0 or float(verificar_renda) < 0:
                    print('A renda informada é inválida. Por favor, digite novamente.')
                    time.sleep(5)
                    os.system('cls')
                else:
                    regex_renda = re.compile(r'^\d+(\.\d{3})*(?:,\d{1,2})?$')
                    if regex_renda.match(self._renda):
                        self._renda = float(self._renda.replace(',', '_').replace('.', '').replace('_', '.'))
                        os.system('cls')
                        return self._renda
                    else:
                        print('A renda informada é inválida. Por favor, digite novamente.\n\nFormatos aceitos: XXXX,XX - X.XXX,XX - XXXX')
                        time.sleep(10)
                        os.system('cls')
    ...
    def _limite_cheque_especial(self) -> float:
        cliente = localizar_cliente_db(cpf_formatado)
        if cliente['Renda'] == 0 or cliente['Renda'] == None:
            self.limite_cheque_especial = 500
            return self.limite_cheque_especial
        else:
            self.limite_cheque_especial = (cliente['Renda'] * 3)
            return self.limite_cheque_especial

    def exibir_dados_pessoais(self) -> str:
        os.system('cls')
        cliente = localizar_cliente_db(cpf_formatado)
        data_nascimento = datetime.strftime(cliente['Data_nascimento'], format='%d/%m/%Y')

        print(f'Dados pessoais de {self._nome}:\n')
        print(f'CPF: {cliente["CPF"]}')
        print(f'RG: {cliente["RG"]}')
        print(f'Data de Nascimento: {data_nascimento}')
        print(f'Gênero: {cliente["Gênero"]}')
    ...
    def depositar_dinheiro(self) -> str:
        while True:
            deposito = input('Quanto você gostaria de depositar? ').strip()
            if not deposito:
                print('Nenhum valor foi informado. Caso queira cancelar a operação digite "Cancelar".')
                time.sleep(5)
                os.system('cls')
            else:
                if deposito == 'Cancelar':
                    print('Operação cancelada com sucesso!')
                    time.sleep(3)
                    os.system('cls')
                    break
                else:
                    verificar_deposito = deposito.replace(',', '_').replace('.', '').replace('_', '.')
                    if float(verificar_deposito) == 0 or float(verificar_deposito) < 0:
                        print('O valor de depósito precisar ser maior do que zero. Caso queira cancelar a operação digite "Cancelar".')
                        time.sleep(5)
                        os.system('cls')
                    else:
                        regex_deposito = re.compile(r'^\d+(\.\d{3})*(?:,\d{1,2})?$')
                        if regex_deposito.match(deposito):
                            deposito = float(deposito.replace(',', '_').replace('.', '').replace('_', '.'))
                            atualizar_deposito_bancario(
                                cpf=cpf_formatado,
                                valor=deposito,
                                data_atualizacao=ContaCorrente._data_hora()
                            )
                            dados_cliente = localizar_cliente_db(cpf_formatado)
                            armanezar_transacao(
                                id_cliente=dados_cliente['ID_Cliente'],
                                valor_transacao=deposito,
                                tipo_transacao="Depósito",
                                protocolo_transacao=self._criar_chave_hash(self._nome, cpf_formatado),
                                data_atualizacao=ContaCorrente._data_hora()
                            )
                            os.system('cls')
                            return self.consultar_saldo()
                        else:
                            print('O depósito informado é inválido. Por favor, digite novamente.\n\nFormatos aceitos: XXXX,XX - X.XXX,XX - XXXX')
                            time.sleep(5)
                            os.system('cls')

    def consultar_saldo(self) -> str:
        saldo = consultar_saldo_bancario(cpf_formatado)
        self._saldo = float(saldo['Saldo'])
    ...
    def transferir_dinheiro(self):
        while True:
            cpf_destino = input('Por favor, informe o CPF do beneficiário: ').strip()
            if cpf_destino == 'Cancelar':
                print('Operação cancelada com sucesso!')
                time.sleep(3)
                os.system('cls')
                break
            ...
            time.sleep(3)
            os.system('cls')
            dados_cliente = localizar_cliente_db(cpf_destino)
            if dados_cliente != 'Cliente não encontrado':
                agencia_destino = input('Por favor, informe o número da agência: ').strip()
                conta_destino = input('Por favor, informe o número da conta: ').strip()
                print('Localizando agência e conta do beneficiário...')
                time.sleep(3)
                os.system('cls')
                dados_conta = localizar_agencia_conta_db(cpf_destino)
                if agencia_destino == dados_conta['Agência'] and conta_destino == dados_conta['Conta']:
                    valor = input(f'Quanto você gostaria de transferir para {dados_cliente["Nome_cliente"]}? ').strip()
                    regex_transferencia = re.compile(r'^\d+(\.\d{3})*(?:,\d{1,2})?$')
                    if regex_transferencia.match(valor):
                        os.system('cls')
                        valor = float(valor.replace(',', '_').replace('.', '').replace('_', '.'))
                        if (dados_conta['Saldo'] + self._limite_cheque_especial()) >= valor:
                            retorno_atualizar_saldo = atualizar_transferencia(cpf_destino, valor, data_atualizacao=ContaCorrente._data_hora(), tipo_operacao='beneficiario')
                            armanezar_transacao(
                                                id_cliente=dados_cliente['ID_Cliente'],
                            ...
                            if retorno_atualizar_saldo == "Operação concluída com sucesso!":
                                    atualizar_transferencia(cpf_formatado, valor, data_atualizacao=ContaCorrente._data_hora())
                                    id_cliente_titular = self._localizar_cliente(cpf_formatado)
                                    armanezar_transacao(
                                    ...
                                    print(f'Transferência concluída com sucesso!\n\nO protocolo da operação é: {self._criar_chave_hash(self._nome, cpf_formatado)}\n\nObrigado e volte sempre!')
                                    time.sleep(10)
                                    os.system('cls')
                                    break
                            else:
                            ...
                            if retorno == 'N':
                                ...
                                if novo_retorno == 'N':
                                    os.system('cls')
                                    print(f'Sendo assim, vamos encerrar o seu atendimento, {self._nome.split()[0].title()}. Obrigado e volte sempre!')
                                    time.sleep(5)
                                    os.system('cls')
                                    break
                                else:
                                ...
                                novo_valor = input('Quanto você gostaria de transferir dessa vez? ').strip()
                                regex_transferencia = re.compile(r'^\d+(\.\d{3})*(?:,\d{1,2})?$')
                                if regex_transferencia.match(valor):
                                    valor = float(valor.replace(',', '_').replace('.', '').replace('_', '.'))
                                    if (dados_conta['Saldo'] + self._limite_cheque_especial()) >= novo_valor:
                                        retorno_atualizar_saldo_novo_valor = atualizar_transferencia(cpf_destino, novo_valor, data_atualizacao=ContaCorrente._data_hora(), tipo_operacao='beneficiario')
                                        if retorno_atualizar_saldo_novo_valor == "Operação concluída com sucesso!":
                                            armanezar_transacao(
                                                    id_cliente=dados_cliente['ID_Cliente'],
                                                    valor_transacao=novo_valor,
                                                    tipo_transacao="Transferência", 
                                                    protocolo_transacao=self._criar_chave_hash(self._nome, cpf_formatado),
                                                    data_atualizacao=ContaCorrente._data_hora()
                                                    )
                                            atualizar_transferencia(self._cpf, novo_valor, data_atualizacao=ContaCorrente._data_hora())
                                            id_cliente_titular = self._localizar_cliente(cpf_formatado)
                                            armanezar_transacao(
                                                    id_cliente=id_cliente_titular['ID_Cliente'],
                                                    valor_transacao=-novo_valor,
                                        ...
                                        time.sleep(5)
                                        os.system('cls')
                                        print('Devido a duas tentativas mal-sucedias de transferir um valor não disponível, estaremos encerrando a sua sessão neste momento.\n\nObrigado e volte sempre!')
                                        time.sleep(5)
                                        os.system('cls')
                                else:
                                    print('O valor informado não pode ser menor ou igual a zero. Por favor, digite novamente. Caso queira cancelar a operação digite "Cancelar".')
                                    time.sleep(5)
                                    os.system('cls')
                    else:
                        print('O valor informado não pode ser menor ou igual a zero. Por favor, digite novamente. Caso queira cancelar a operação digite "Cancelar".')
        ...
        while True:
            ...
            if valor == 'Cancelar':
                ...
                if not valor.strip():
                    ...
                    regex_saque = re.compile(r'^\d+(\.\d{3})*(?:,\d{1,2})?$')
                    if regex_saque.match(valor):
                        valor = float(valor.replace(',', '_').replace('.', '').replace('_', '.'))
                        saldo_bancario = consultar_saldo_bancario(cpf=cpf_formatado)
                        if saldo_bancario['Saldo'] + self._limite_cheque_especial() >= valor:
                            dados_cliente = localizar_cliente_db(cpf=cpf_formatado)
                            atualizar_saque(cpf=cpf_formatado, valor=valor, data_atualizacao=ContaCorrente._data_hora())
                            armanezar_transacao(
                                id_cliente=dados_cliente['ID_Cliente'],
                                valor_transacao=valor,
                            ...
                            if retorno == 'N':
                                ...
                                novo_valor = input('Quanto você gostaria de sacar dessa vez? ').strip()
                                regex_saque = re.compile(r'^\d+(\.\d{3})*(?:,\d{1,2})?$')
                                if regex_saque.match(valor):
                                    valor = float(valor.replace(',', '_').replace('.', '').replace('_', '.'))
                                    if (saldo_bancario['Saldo'] + self._limite_cheque_especial()) >= novo_valor:
                                        dados_cliente = localizar_cliente_db(cpf=self._cpf)
                                        atualizar_saque(cpf=cpf_formatado, valor=novo_valor, data_atualizacao=ContaCorrente._data_hora())
                                        armanezar_transacao(
                                            id_cliente=dados_cliente['ID_Cliente'],
                                            valor_transacao=valor,
                                            tipo_transacao="Saque",
                                        ...
                                        print(f'Saque concluído com sucesso!\n\nO protocolo da operação é: {self._criar_chave_hash(self._nome, cpf_formatado)}\n\nO seu saldo atual é: {self.consultar_saldo()}\n\nObrigado e volte sempre!')
                                        time.sleep(10)
                                        os.system('cls')
                                        break
                                    else:
                                        ...
                                        time.sleep(5)
                                        os.system('cls')
                                        print('Devido a duas tentativas mal-sucedias de sacar um valor não disponível, estaremos encerrando a sua sessão neste momento.\n\nObrigado e volte sempre!')
                                        time.sleep(5)
                                        os.system('cls')
                                else:
                                    print('O valor informado não pode ser menor ou igual a zero. Por favor, digite novamente. Caso queira cancelar a operação digite "Cancelar".')
